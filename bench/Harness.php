    ...
    public function writeMarkdown(string $filepath): void
    {
        $scenarios = [
            'literal_short' => [
                ...
                'desc' => 'A simple substring search (`\'hello\'` vs `/hello/`). Measures end-to-end PHP binding throughput for the most basic pattern type.',
            ],
            ...
            'tokenization' => [
                'name' => 'Tokenization',
                'desc' => 'Repeatedly tokenizes a long comma-separated string (1000 tokens). Stress-tests the binding\'s ability to handle many match operations.',
            ],
            'substitution' => [
                'name' => 'Substitution',
        ...
        $md = <<<MARKDOWN
# Benchmarks: PHP Binding — libsnobol4 vs PCRE2

> **What these benchmarks measure:** The **end-to-end PHP binding** (PatternHelper +
> compilation + VM + JIT). They include PHP-to-C overhead, per-call pattern
> compilation, and result construction — not raw C engine throughput.
>
> For a fair C-level comparison see the dedicated C microbenchmark suite
> (`bench/c/`). Results at this layer are driven largely by PHP binding
> architecture; the raw C engine may be significantly faster than these numbers
> suggest.

This document presents head-to-head benchmark results comparing the libsnobol4
PHP binding against PCRE2 across representative pattern matching workloads.
...
> **Note:** These ratios reflect the PHP binding overhead (per-call compilation,
> VM construction, result array building) which PCRE2 does not incur via
> `preg_*()`. See [bench/c/](bench/c/) for C-level microbenchmarks that isolate
...
- Tokenization of long subjects: PHP binding overhead at every delimiter creates a significant gap
...
These are **PHP-binding end-to-end numbers**. They include pattern compilation,
VM setup, and result construction on every call. PCRE2 benefits from
pre-compilation (pattern compiled once by PHP's `preg_*` internals) and direct
C-to-C execution without PHP array construction overhead. See the
[C microbenchmark suite](bench/c/) for ratios at the C engine level.
