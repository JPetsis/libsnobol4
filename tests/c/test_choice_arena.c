...
static void test_arena_unit(void) {
   ...
   * pages (each page is 4096 bytes; records are ~16 bytes -> >250 records
   * spans pages). */
  const int N = 4000;
  for (int i = 0; i < N; i++) {
  ...
  test_assert(a->total_used == (size_t)N * (sizeof(uint32_t) + 2 * sizeof(uint32_t)),
  ...
  for (int i = N - 1; i >= 0; i--) {
    /* Trailing size word is at the page tail; payload is just after the
     * leading size word of the record. */
    uint32_t footprint =
        *(uint32_t *)(a->cur->data + a->cur->used - sizeof(uint32_t));
    uint8_t *rec_base = a->cur->data + a->cur->used - footprint;
    uint32_t *payload = (uint32_t *)(rec_base + sizeof(uint32_t));
