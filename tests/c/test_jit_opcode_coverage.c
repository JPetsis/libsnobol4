...
static uint16_t add_class_ch(uint8_t *bc, size_t bc_len, uint8_t ch) {
  uint32_t cc = ((uint32_t)bc[bc_len - 4] << 24) |
                ((uint32_t)bc[bc_len - 3] << 16) |
  ...
  /* update class_count */
  bc[bc_len - 4] = (uint8_t)(sid >> 24);
  bc[bc_len - 3] = (uint8_t)(sid >> 16);
  bc[bc_len - 2] = (uint8_t)(sid >> 8);
  bc[bc_len - 1] = (uint8_t)(sid);
  return sid;
}

/* Set up JIT on a VM (already has bc/bc_len set) and warm up. Returns ctx. */
static SnobolJitContext *setup_jit_vm(VM *vm, int hotness,
                                      const char *warmup_s) {
...
static void test_rem_jit(void) {
  if (!jit_is_supported()) {
    test_assert(true, "OP_REM JIT: skipped (non-ARM64)");
    return;
  }
  snobol_jit_init();
...
static void test_rpos_jit(void) {
  if (!jit_is_supported()) {
    test_assert(true, "OP_RPOS JIT: skipped (non-ARM64)");
    return;
  }
  snobol_jit_init();
  snobol_jit_reset_stats();
  ...
  size_t ip = 0;
  emit_lit(bc, &ip, "ab", 2);
  cov_u8(bc, &ip, OP_RPOS);
  cov_u32(bc, &ip, 0); /* n=0: must be at end */
  cov_u8(bc, &ip, OP_ACCEPT);
...
static void test_rtab_jit(void) {
  if (!jit_is_supported()) {
    test_assert(true, "OP_RTAB JIT: skipped (non-ARM64)");
    return;
  }
  snobol_jit_init();
  snobol_jit_reset_stats();
  ...
  vm.s = "abc";
  vm.len = 3;
  test_assert(vm_exec(&vm),
              "OP_RTAB JIT: 'abc' after 'a' RTAB(0) advances to end");
...
static void test_fence_jit(void) {
  if (!jit_is_supported()) {
    test_assert(true, "FENCE JIT: skipped (non-ARM64)");
    return;
  }
  snobol_jit_init();
  snobol_jit_reset_stats();
...
static void test_label_jit(void) {
  if (!jit_is_supported()) {
    test_assert(true, "LABEL JIT: skipped (non-ARM64)");
    return;
  }
  snobol_jit_init();
  snobol_jit_reset_stats();
  ...
  emit_lit(bc, &ip, "ab", 2);
  size_t label_ip = ip;
  cov_u8(bc, &ip, OP_LABEL);
  cov_u16(bc, &ip, 0);
  cov_u8(bc, &ip, OP_ACCEPT);

  /* New-format tail (last 16 bytes of 256-byte bc) */
  bc[240] = bc[241] = bc[242] = bc[243] = 0; /* class_count=0 */
  bc[244] = (uint8_t)(label_ip >> 24);
  bc[245] = (uint8_t)(label_ip >> 16);
  bc[246] = (uint8_t)(label_ip >> 8);
  bc[247] = (uint8_t)(label_ip);
  bc[248] = bc[249] = bc[250] = 0;
  bc[251] = 1; /* label_count=1 */
  bc[252] = (uint8_t)(SNBL_MAGIC >> 24);
  bc[253] = (uint8_t)(SNBL_MAGIC >> 16);
  bc[254] = (uint8_t)(SNBL_MAGIC >> 8);
  bc[255] = (uint8_t)(SNBL_MAGIC);
...
static void test_emit_cap_jit(void) {
  if (!jit_is_supported()) {
    test_assert(true, "EMIT_CAPTURE JIT: skipped (non-ARM64)");
    return;
  }
  snobol_jit_init();
  snobol_jit_reset_stats();
  ...
  bc[252] = bc[253] = bc[254] = bc[255] = 0;
  size_t ip = 0;
  cov_u8(bc, &ip, OP_CAP_START);
  cov_u8(bc, &ip, 0);
  emit_lit(bc, &ip, "a", 1);
  cov_u8(bc, &ip, OP_CAP_END);
  cov_u8(bc, &ip, 0);
  cov_u8(bc, &ip, OP_EMIT_CAPTURE);
  cov_u8(bc, &ip, 0);
  cov_u8(bc, &ip, OP_ACCEPT);
...
static void test_bal_jit(void) {
  if (!jit_is_supported()) {
    test_assert(true, "BAL JIT: skipped (non-ARM64)");
    return;
  }
  snobol_jit_init();
  snobol_jit_reset_stats();
...
static bool test_eval_cb(int fn_id, const char *s, size_t start, size_t end,
                         void *u) {
  (void)fn_id;
  (void)s;
  (void)start;
  ...
  (void)u;
  g_eval_calls++;
  if (g_eval_fail_after >= 0 && g_eval_calls > g_eval_fail_after)
    return false;
  return true;
}
...
static void test_eval_jit(void) {
  if (!jit_is_supported()) {
    test_assert(true, "EVAL JIT: skipped (non-ARM64)");
    return;
  }
  snobol_jit_init();
  snobol_jit_reset_stats();
  ...
  test_assert(g_eval_calls > 0,
              "EVAL JIT: host callback invoked during warmup");
  ...
  vm.len = 2;
  test_assert(!vm_exec(&vm),
              "EVAL JIT: callback returns false → pattern fails");
...
static void test_table_get_jit(void) {
  if (!jit_is_supported()) {
    test_assert(true, "TABLE_GET JIT: skipped (non-ARM64)");
    return;
  }
  snobol_jit_init();
  snobol_jit_reset_stats();
  ...
  cov_u8(bc, &ip, OP_CAP_END);
  cov_u8(bc, &ip, 0);
  /* OP_TABLE_GET: table_id=0(u16), key_reg=0(u8), dest_reg=0(u8),
   * name_len=0(u8) */
  ...
  test_assert(table_set(tbl, "key", "val1"),
              "TABLE_GET JIT: table_set succeeded");
  uint16_t tid = 0;
  test_assert(vm_register_table(&vm, tbl, &tid),
              "TABLE_GET JIT: vm_register_table succeeded");
...
static void test_table_set_jit(void) {
  if (!jit_is_supported()) {
    test_assert(true, "TABLE_SET JIT: skipped (non-ARM64)");
    return;
  }
  snobol_jit_init();
  snobol_jit_reset_stats();
  ...
  size_t d0 = ip;
  bc[ip++] = 'k';
  bc[off0] = (uint8_t)(d0 >> 24);
  bc[off0 + 1] = (uint8_t)(d0 >> 16);
  bc[off0 + 2] = (uint8_t)(d0 >> 8);
  bc[off0 + 3] = (uint8_t)(d0);
  cov_u8(bc, &ip, OP_CAP_END);
  cov_u8(bc, &ip, 0);
  ...
  size_t d1 = ip;
  bc[ip++] = 'v';
  bc[off1] = (uint8_t)(d1 >> 24);
  bc[off1 + 1] = (uint8_t)(d1 >> 16);
  bc[off1 + 2] = (uint8_t)(d1 >> 8);
  bc[off1 + 3] = (uint8_t)(d1);
  cov_u8(bc, &ip, OP_CAP_END);
  cov_u8(bc, &ip, 1);
  ...
  uint16_t tid = 0;
  test_assert(vm_register_table(&vm, tbl, &tid),
              "TABLE_SET JIT: vm_register_table succeeded");
...
static void test_dynamic_jit(void) {
  if (!jit_is_supported()) {
    test_assert(true, "DYNAMIC JIT: skipped (non-ARM64)");
    return;
  }
  snobol_jit_init();
  snobol_jit_reset_stats();
   ...
   *   OP_DYNAMIC_DEF + source_len(u32)=1 + "A" + bc_len(u32)=2 + OP_LIT"A" +
   * OP_ACCEPT OP_DYNAMIC (pattern_reg=0) Then outer: OP_LIT "X" OP_DYNAMIC 0
   * OP_ACCEPT
   */
  uint8_t inner_bc[64];
  memset(inner_bc, 0, sizeof(inner_bc));
  size_t iip = 0;
