 ...
 * one is bound (Task 3.2), that the arena is reset/reclaimed cleanly, and that
...
/* Task 3.2: AST nodes are bump-allocated from a bound arena. */
void test_arena_allocation(void) {
  ...
  test_assert(arena.ptr == arena.base, "arena reset returns bump pointer to base");
...
/* Task 3.2: without a bound arena the default allocator is used and the tree
...
/* Task 3.2: the full public compile path (which now binds a per-compile arena
...
void test_arena_public_compile(void) {
  ...
  snobol_pattern_t *pat = snobol_pattern_compile_ex(ctx, src, strlen(src), 0, &err);
