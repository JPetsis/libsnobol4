...
static void test_cache_eviction_stress(void) {
  ...
  test_assert(ctx4 != NULL,
              "Eviction stress: 4th context acquired after eviction");

  /* Acquire 5th pattern → triggers another eviction */
  SnobolJitContext *ctx5 = snobol_jit_acquire_context(bcs[4], lens[4]);
  test_assert(ctx5 != NULL,
              "Eviction stress: 5th context acquired after second eviction");
  ...
  test_assert(ctx3b != NULL,
              "Eviction stress: re-acquire after potential eviction is safe");
  ...
  if (ctx3b)
    snobol_jit_release_context(ctx3b);
...
static void test_backtrack_alternation_fixture(void) {
  ...
  /* Patch targets */
  size_t tmp = patch_a;
  bc[tmp++] = (uint8_t)(a_ip >> 24);
  bc[tmp++] = (uint8_t)(a_ip >> 16);
  bc[tmp++] = (uint8_t)(a_ip >> 8);
  bc[tmp++] = (uint8_t)(a_ip);
  tmp = patch_b;
  bc[tmp++] = (uint8_t)(b_ip >> 24);
  bc[tmp++] = (uint8_t)(b_ip >> 16);
  bc[tmp++] = (uint8_t)(b_ip >> 8);
  bc[tmp++] = (uint8_t)(b_ip);
  tmp = patch_end;
  bc[tmp++] = (uint8_t)(end_ip >> 24);
  bc[tmp++] = (uint8_t)(end_ip >> 16);
  bc[tmp++] = (uint8_t)(end_ip >> 8);
  bc[tmp++] = (uint8_t)(end_ip);
  (void)jmp_ip;
  (void)split_ip;
  ...
  bool ok_x = vm_run(&vm);
  test_assert(ok_x,
              "Backtrack alternation fixture: 'x' matches via primary path");
  ...
  bool ok_y = vm_run(&vm);
  test_assert(ok_y,
              "Backtrack alternation fixture: 'y' matches via backtrack path");
  ...
  test_assert(
      true,
      "Backtrack alternation fixture: 200 iterations complete without crash");
...
void test_jit_cache_suite(void) {
  ...
  test_assert(snobol_jit_get_stats()->cache_hits_total == 1,
              "Stats reflect cache hit");
