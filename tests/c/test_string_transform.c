...
void test_string_transform_suite(void) {
    ...
    snobol_dupl(alpha_beta, 4, 2, &b);
    test_assert(b.len == 8 && memcmp(b.data, "\xCE\xB1\xCE\xB2\xCE\xB1\xCE\xB2", 8) == 0,
                "DUPL: 'αβ' x2 = correct UTF-8");

    /* --- REVERSE --- */
    snobol_reverse("hello", 5, &b);
    test_assert(b.len == 5 && memcmp(b.data, "olleh", 5) == 0,
                "REVERSE: 'hello' → 'olleh'");

    snobol_reverse("a", 1, &b);
    ...
    snobol_reverse(abc_greek, 6, &b);
    test_assert(b.len == 6 &&
                memcmp(b.data, "\xCE\xB3\xCE\xB2\xCE\xB1", 6) == 0,
