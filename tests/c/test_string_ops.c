...
void test_cov_misc_string_fn(void) {
  ...
  test_assert(snobol_dupl("ab", 2, 0, &out) && out.len == 0, "dupl 0");
  test_assert(snobol_dupl("ab", 2, 3, &out) && out.len == 6, "dupl 3");
  test_assert(snobol_dupl(NULL, 2, 3, &out) && out.len == 0, "dupl NULL");
  ...
  test_assert(snobol_replace_char("abca", 4, "a", 1, "z", 1, &out) &&
                  out.len == 4 && memcmp(out.data, "zbcz", 4) == 0,
              "replace_char all");

  /* LPAD / RPAD with multibyte pad and width. */
  test_assert(snobol_lpad("ab", 2, 5, '0', &out) && out.len == 5 &&
                  memcmp(out.data, "000ab", 5) == 0,
              "lpad");
  test_assert(snobol_rpad("ab", 2, 5, '0', &out) && out.len == 5 &&
                  memcmp(out.data, "ab000", 5) == 0,
              "rpad");
  test_assert(snobol_lpad("abcde", 5, 3, '0', &out) && out.len == 5,
              "lpad no-pad passthrough");
  test_assert(snobol_rpad("abcde", 5, 3, '0', &out) && out.len == 5,
              "rpad no-pad passthrough");
  test_assert(snobol_lpad(NULL, 0, 5, '0', &out), "lpad NULL empty");

  /* CHAR / ORD incl. multibyte. */
  test_assert(snobol_char_fn(0x20AC, &out) && out.len == 3, "char euro");
  ...
  uint32_t cp = 0;
  test_assert(snobol_ord("\xE2\x82\xAC", 3, &cp) && cp == 0x20AC,
              "ord euro codepoint");
  test_assert(!snobol_ord("", 0, &cp), "ord empty");
  ...
  test_assert(snobol_upper("a\xE4\xB8\xADz", 5, &out) && out.len == 5 &&
                  memcmp(out.data, "A\xE4\xB8\xADZ", 5) == 0,
              "upper non-foldable codepoint passthrough");
  test_assert(snobol_size("abc", 3) == 3, "size");
  test_assert(snobol_size(NULL, 0) == 0, "size NULL");
...
void test_cov_misc_string_round2(void) {
  ...
  test_assert(snobol_reverse("\xE2\x82", 2, &out) && out.len == 2,
              "reverse truncated UTF-8");
  test_assert(snobol_upper("a\xE2", 2, &out) && out.len == 2,
  ...
  test_assert(snobol_char_fn(0x7F, &out) && out.len == 1, "char 0x7F");
  test_assert(snobol_char_fn(0x80, &out) && out.len == 2, "char 0x80");
  test_assert(snobol_char_fn(0x800, &out) && out.len == 3, "char 0x800");
  test_assert(snobol_char_fn(0x10000, &out) && out.len == 4, "char 0x10000");

  /* ORD: invalid UTF-8 lead byte. */
  uint32_t cp = 0;
  test_assert(!snobol_ord("\xFF", 1, &cp), "ord invalid lead byte");
  test_assert(!snobol_ord("\xE2", 1, &cp), "ord truncated sequence");
  test_assert(snobol_ord("\xC3\xA9", 2, &cp) && cp == 0xE9,
              "ord 2-byte codepoint");

  /* LPAD/RPAD with a multibyte pad codepoint. */
  test_assert(snobol_lpad("x", 1, 3, 0x20AC, &out) && out.len == 7,
              "lpad multibyte pad");
  test_assert(snobol_rpad("x", 1, 3, 0x20AC, &out) && out.len == 7,
              "rpad multibyte pad");

  /* SUBSTR walk with 1-based multibyte positions. */
  test_assert(snobol_substr("\xE2\x82\xACxy", 5, 1, 1, &out) && out.len == 3,
              "substr multibyte codepoint");
...
void test_cov_misc_round3_string(void) {
  ...
  test_assert(!snobol_trim(NULL, 0, &out), "trim(NULL)");
  test_assert(!snobol_dupl("ab", 2, 3, NULL), "dupl(NULL out)");
  test_assert(!snobol_reverse("ab", 2, NULL), "reverse(NULL out)");
  test_assert(!snobol_substr("abc", 3, 5, 1, &out), "substr beyond end");
  test_assert(snobol_replace("abc", 3, "b", 1, "", 0, &out) && out.len == 2 &&
                  memcmp(out.data, "ac", 2) == 0,
              "replace with empty replacement");
  test_assert(!snobol_char_fn(0x20AC, NULL), "char_fn(NULL out)");
  uint32_t cp = 0;
  ...
  test_assert(snobol_lpad("ab", 2, 2, '0', &out) && out.len == 2,
              "lpad width == len");
  test_assert(snobol_rpad("ab", 2, 2, '0', &out) && out.len == 2,
              "rpad width == len");
  test_assert(snobol_reverse("a\xC3\xA9", 3, &out) && out.len == 3,
              "reverse multibyte");
  test_assert(snobol_replace_char("abc", 3, "a", 1, "", 0, &out) &&
                  out.len == 3,
...
void test_cov_misc_round4_string(void) {
  ...
  test_assert(snobol_lpad("x", 1, 2, 0x80, &out) && out.len == 3,
              "lpad 2-byte pad codepoint");
  test_assert(snobol_rpad("x", 1, 2, 0x80, &out) && out.len == 3,
              "rpad 2-byte pad codepoint");
  test_assert(snobol_char_fn(0x7FF, &out) && out.len == 2, "char 0x7FF");
  uint32_t cp = 0;
  test_assert(snobol_ord("\xE4\xB8\xAD", 3, &cp) && cp == 0x4E2D,
