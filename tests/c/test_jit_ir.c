...
static void test_ir_dce_keeps_side_effecting(void) {
    ...
    jit_ir_region_t *r = jit_ir_region_new();
    test_assert(r != NULL, "region allocated");
    jit_ir_append(r, JIT_IR_ACCEPT, JIT_IR_FLAG_SIDE_EFFECT, 0,
...
static void test_ir_dce_keeps_used_pure(void) {
    ...
    uint8_t v1 = jit_ir_alloc_vreg(r);
    jit_ir_append(r, JIT_IR_NOP, JIT_IR_FLAG_PURE, 0, v1, 0, 0);
    jit_ir_append(r, JIT_IR_ACCEPT, JIT_IR_FLAG_SIDE_EFFECT, 1,
                  JIT_IR_VREG_NONE, v1, 0);
...
static void test_ir_copy_prop_folds_copy(void) {
    ...
    jit_ir_append(r, JIT_IR_ACCEPT, JIT_IR_FLAG_SIDE_EFFECT,  2,
                  JIT_IR_VREG_NONE, v2, 0);
    ...
    for (size_t i = 0; i < r->count; i++) {
        if (r->instrs[i].opcode == JIT_IR_NOP)    found_nop    = true;
        if (r->instrs[i].opcode == JIT_IR_ACCEPT)  found_accept = true;
    }
...
static void test_ir_copy_prop_noop_when_no_copies(void) {
    ...
    jit_ir_region_t *r = jit_ir_region_new();
    test_assert(r != NULL, "region allocated");
    jit_ir_append(r, JIT_IR_ACCEPT, JIT_IR_FLAG_SIDE_EFFECT, 0, JIT_IR_VREG_NONE, 0, 0);
    jit_ir_append(r, JIT_IR_FAIL,   JIT_IR_FLAG_SIDE_EFFECT, 1, JIT_IR_VREG_NONE, 0, 0);
    jit_ir_copy_propagation(r);
...
static void test_ir_vreg_limit_256(void) {
    ...
    for (int i = 0; i < 255; i++) {
        ...
        if (v == JIT_IR_VREG_NONE) { all_valid = false; break; }
    }
...
static void test_ir_dump_smoke(void) {
    ...
    if (sink) { jit_ir_dump(r, sink); fclose(sink); }
