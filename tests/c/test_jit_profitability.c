...
static void test_profitability_cold_skip(void) {
  ...
  vm1.bc_len = len1;
  test_assert(
      !snobol_jit_should_compile(&vm1, 0, &cfg, false),
  ...
  test_assert(snobol_jit_should_compile(&vm2, 0, &cfg, false),
              "Hot pattern: two-literal pattern enables JIT (useful >= "
              "min_useful_ops)");
}
...
static void test_profitability_backtrack_skip(void) {
  ...
  test_assert(snobol_jit_should_compile(&vm, 0, &cfg, false),
              "Backtrack allow: with skip_backtrack_heavy=false and "
              "min_useful_ops=0, SPLIT pattern passes gate");
}
