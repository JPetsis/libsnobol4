 ...
 * 5.3  'ab' | 'c'        → bytecode retains OP_SPLIT (multi-char arm, ineligible)
 * 5.4  ANY(cc)|NOTANY(cc2) would retain OP_SPLIT; we test NOTANY arm is not fused
...
static bool run_match(const uint8_t *bc, size_t bc_len, const char *s) {
    ...
    if (vm.choices) { free(vm.choices); vm.choices = NULL; }
    return result;
}
...
static void test_fusion_ab_has_any_no_split(void) {
    ...
    if (rc == 0 && bc) {
        ...
        test_assert(any_count >= 1,   "fusion: at least one OP_ANY in 'a'|'b' bytecode");
        free(bc);
    }
}
...
static void test_fusion_ab_semantics(void) {
    ...
    int rc = compile_ast_to_bytecode_c(alt, false, &bc, &bc_len);
    snobol_ast_free(alt);
    if (rc != 0 || !bc) { test_assert(false, "compile failed"); return; }
...
static void test_fusion_notany_not_fused(void) {
    ...
    int split_count = count_opcode_in_range(bc, bc_len, (uint8_t)OP_SPLIT);
    test_assert(split_count >= 1, "LIT|NOTANY retains OP_SPLIT (NOTANY arm not fused)");
    free(bc);
}
...
static void test_fusion_three_way_one_any(void) {
    ...
    ast_node_t *bc_node = snobol_ast_create_alt(
        snobol_ast_create_lit("b", 1),
        snobol_ast_create_lit("c", 1)
    );
    ast_node_t *alt3 = snobol_ast_create_alt(
        snobol_ast_create_lit("a", 1),
        bc_node
    );
