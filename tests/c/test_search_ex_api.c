...
void test_search_ex_api_suite(void) {
    ...
    /* Stateful search matches non-stateful search results */
    {
        ...
        if (pat) {
            ...
            const char *ref_output = ref ? snobol_match_get_output(ref, &ref_output_len) : NULL;
            char ref_buf[64] = {0};
            if (ref_output) {
            ...
            snobol_pattern_search_state_t *state =
                snobol_pattern_search_state_create(snobol_pattern_get_bc(pat), snobol_pattern_get_bc_len(pat));
    ...
    /* Hot loop: JIT still fires through the stateful path */
    {
        ...
        if (pat) {
            ...
            if (state) {
                /* warmup */
                for (int i = 0; i < 100; i++) {
                    snobol_match_t *m = snobol_pattern_search_ex(state,
                        "aaaaaaaaaaaaaaaaaa", 18, 0);
                    (void)m;
                }
#ifdef SNOBOL_JIT
                ...
                test_assert(after > before,
                            "JIT fires through stateful path");
#endif
                snobol_pattern_search_state_destroy(state);
            }
            snobol_pattern_free(pat);
    ...
    {
        snobol_pattern_search_state_t *s = snobol_pattern_search_state_create(NULL, 0);
        test_assert(s == NULL, "state create with NULL pattern returns NULL");
        if (s) snobol_pattern_search_state_destroy(s);
    }
}
