...
static void test_cfg_split_chain_blocks(void) {
  ...
  size_t pos_split0 = w;
  bc[w++] = OP_SPLIT;
  size_t patch_a_off = w;
  emit_u32_cfg(bc, &w, 0); /* arm-a placeholder */
  size_t patch_s2_off = w;
  emit_u32_cfg(bc, &w, 0); /* arm-b placeholder */

  size_t a_ip = w;
  bc[w++] = OP_ANY;
  bc[w++] = 0;
  bc[w++] = 1; /* ANY set_id=1 ('a') */
  size_t patch_jmp_a = w;
  bc[w++] = OP_JMP;
  emit_u32_cfg(bc, &w, 0); /* forward JMP to merge */

  size_t split2_ip = w;
  bc[w++] = OP_SPLIT;
  size_t patch_b_off = w;
  emit_u32_cfg(bc, &w, 0);
  size_t patch_c_off = w;
  emit_u32_cfg(bc, &w, 0);

  size_t b_ip = w;
  bc[w++] = OP_ANY;
  bc[w++] = 0;
  bc[w++] = 2; /* ANY set_id=2 ('b') */
  ...
  test_assert(delta >= 4, "3-arm SPLIT chain: jit_blocks_compiled_total "
                          "incremented by >= 4 blocks");
  test_assert(stats->entries_total > 0, "3-arm SPLIT chain: JIT entries > 0");
...
static void test_cfg_split_backtrack_restores_state(void) {
  ...
  bc[pa] = (uint8_t)(a_ip >> 24);
  bc[pa + 1] = (uint8_t)(a_ip >> 16);
  bc[pa + 2] = (uint8_t)(a_ip >> 8);
  bc[pa + 3] = (uint8_t)(a_ip);
  bc[pb] = (uint8_t)(b_ip >> 24);
  bc[pb + 1] = (uint8_t)(b_ip >> 16);
  bc[pb + 2] = (uint8_t)(b_ip >> 8);
  bc[pb + 3] = (uint8_t)(b_ip);
  ...
  test_assert(matched,
              "SPLIT backtrack: 'b' matches via arm-b after arm-a fail");
...
static void test_cfg_arbno_loop_compiled(void) {
  ...
  bc[w++] = OP_SPLIT;
  size_t pa = w;
  emit_u32_cfg(bc, &w, 0); /* arm-a = body */
  size_t pb = w;
  emit_u32_cfg(bc, &w, 0); /* arm-b = after */
  ...
  /* Patch SPLIT */
  bc[pa] = (uint8_t)(body_ip >> 24);
  bc[pa + 1] = (uint8_t)(body_ip >> 16);
  bc[pa + 2] = (uint8_t)(body_ip >> 8);
  bc[pa + 3] = (uint8_t)(body_ip);
  bc[pb] = (uint8_t)(after_ip >> 24);
  bc[pb + 1] = (uint8_t)(after_ip >> 16);
  bc[pb + 2] = (uint8_t)(after_ip >> 8);
  bc[pb + 3] = (uint8_t)(after_ip);
  (void)jmp_ip;
