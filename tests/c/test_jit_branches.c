...
static void test_jit_split_in_region(void) {
  ...
  test_assert(stats->compilations_total > 0,
              "Should have compiled region with SPLIT");
  ...
  test_assert(stats->choice_bytes_total > 0,
              "Should have recorded choice bytes");
...
static void test_jit_split_backtrack_boundary(void) {
  ...
  /* Patch targets */
  size_t p = split_ip + 1;
  emit_u32(bc, &p, (uint32_t)a_ip);
  p = split_ip + 5;
  emit_u32(bc, &p, (uint32_t)b_ip);
  p = jmp_ip + 1;
  emit_u32(bc, &p, (uint32_t)end_ip);
  ...
  /* Final run: JIT executes SPLIT (pushes choice for b), takes a (fails on
   * 'x'), interpreter backtracks to b (succeeds on 'b'). */
  snobol_jit_reset_stats();
  vm.ip = 0;
  vm.pos = 0;
  bool ok = vm_run(&vm);
  ...
  test_assert(vm.pos == 1,
              "SPLIT backtrack: pos should be 1 after consuming 'b'");
...
static void test_jit_split_backward_terminates_region(void) {
   ...
   *   [10] OP_ACCEPT
   *
   * b=0 < split_ip=5 → backward branch → guard fires → region ends before
   * SPLIT. Interpreter processes SPLIT: push choice(0, pos), set ip=10 (ACCEPT)
   * → match. Subject "a" (len=1): LEN 1 succeeds (pos→1), SPLIT takes a=10
   * (ACCEPT). */
  uint8_t bc[32] = {0};
  size_t ip = 0;
