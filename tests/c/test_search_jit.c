 ...
 *   - Verify correctness when search fast paths and compiled JIT regions interact.
...
static int run_search_n(const uint8_t *bc, size_t bc_len,
                         const char *subject, int n) {
    ...
    for (int i = 0; i < n; i++) {
        ...
        bool ok = snobol_search_exec(&vm, subject, strlen(subject),
                                      0, &meta, &result, NULL);
        if (ok) found++;
#ifdef SNOBOL_JIT
        if (vm.jit.ctx) snobol_jit_release_context(vm.jit.ctx);
#endif
    }
    return found;
}
...
static void test_search_mode_profitability_lower_threshold(void) {
    ...
    bool search_ok  = snobol_jit_should_compile(&vm, 0, snobol_jit_get_config(), true);
    /* Anchored-mode should deny (min_useful_ops=2 but we only have 1 useful op) */
    bool anchored_ok = snobol_jit_should_compile(&vm, 0, snobol_jit_get_config(), false);

    test_assert(search_ok,  "search_mode=true allows single-op pattern compilation");
    test_assert(!anchored_ok, "search_mode=false denies single-op with min_useful_ops=2");
...
static size_t jt_build_char_class_op(uint8_t *bc, uint8_t op, char c) {
    ...
    /* charclass data at ip=4: range_count(u16)=1, case_flag(u16)=0, range[c,c](8 bytes) */
    size_t class_data_off = ip;
    bc[ip++] = 0; bc[ip++] = 1;   /* range_count = 1 */
    bc[ip++] = 0; bc[ip++] = 0;   /* case_flag = 0 */
...
static void test_search_no_jit_fallback(void) {
    ...
    snobol_search_result_t result;
    bool ok = snobol_search_exec(&vm, "say hello world", 15, 0, &meta, &result, NULL);
