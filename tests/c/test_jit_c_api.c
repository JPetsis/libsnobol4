...
void test_jit_c_api_suite(void) {
    ...
    /* snobol_context_create() initialises JIT → stats non-NULL */
    {
        ...
        test_assert(stats != NULL, "snobol_jit_get_stats() returns non-NULL after context create");
        (void)stats;
    }
    ...
    /* snobol_pattern_search() finds literal in subject */
    {
        ...
        if (pat) {
            snobol_match_t *m = snobol_pattern_search(pat, "the quick brown fox jumps", 24);
    ...
    /* snobol_pattern_search() produces same result as snobol_pattern_match() at pos 0 */
    {
        snobol_context_t *ctx = snobol_context_create();
        char *err = NULL;
        ...
        if (pat) {
            ...
            test_assert(m1 != NULL && m2 != NULL, "both match and search return non-NULL");
            if (m1 && m2) {
                test_assert(snobol_match_success(m1), "match succeeds for 'hello'");
                test_assert(snobol_match_success(m2), "search succeeds for 'hello' at pos 0");
    ...
    /* snobol_pattern_search() with SPAN delimiter */
    {
        snobol_context_t *ctx = snobol_context_create();
        char *err = NULL;
        snobol_pattern_t *pat = snobol_pattern_compile(ctx, "SPAN('abc')", 11, &err);
    ...
    /* snobol_pattern_compile_ex() with SNOBOL_FLAG_SEARCH_MODE flag */
    {
        snobol_context_t *ctx = snobol_context_create();
        char *err = NULL;
        snobol_pattern_t *pat = snobol_pattern_compile_ex(ctx, "'test'", 6,
                                                           SNOBOL_FLAG_SEARCH_MODE, &err);
        test_assert(pat != NULL, "compile ex with SEARCH_MODE flag succeeds");
        if (pat) {
            snobol_match_t *m = snobol_pattern_search(pat, "this is a test string", 21);
    ...
    {
        snobol_match_result_t *r = snobol_match("'test'", 6, "test subject", 12,
                                                  SNOBOL_FLAG_SEARCH_MODE);
        test_assert(r != NULL, "snobol_match with SEARCH_MODE flag returns non-NULL");
        if (r) {
            test_assert(r->success, "snobol_match with SEARCH_MODE flag succeeds at pos 0");
