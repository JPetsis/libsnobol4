...
static bool capture_count_consistent(const char *pat_str, size_t pat_len,
    ...
    bool ok = true;
    if (s1 && s2) {
        ok = (clen1 == clen2) && (c1 == NULL || c2 == NULL || memcmp(c1, c2, clen1) == 0);
    } else if (s1 != s2) {
        ok = false;
    }
...
static void run_random_trial(unsigned char *pat_buf, size_t pat_len,
    ...
    if (!match_idempotent((const char *)pat_buf, plen,
...
void test_property_based_suite(void) {
    ...
    for (int i = 0; i < trials; i++) {
        ...
        run_random_trial(pat_buf, plen, sub_buf, slen,
                         &idempotent_ok, &capture_ok);
    }

    test_assert(idempotent_ok >= 0,
                "Match idempotency holds across random inputs");
    test_assert(capture_ok >= 0,
                "Capture count consistent across random inputs");
    ...
    for (int i = 0; i < n; i++) {
        if (substitution_roundtrip(
                patterns[i], strlen(patterns[i]),
                subjects[i], strlen(subjects[i]),
                templates[i], strlen(templates[i]))) {
            sub_ok++;
        }
    }
    test_assert(sub_ok > 0, "Substitution round-trips succeed for known-good patterns");
}
