...
void test_pattern_abort_suite(void) {
    ...
    /* ABORT in alternation should prevent backtracking */
    {
         ...
         */
        uint8_t bc[] = {
            OP_SPLIT, 0,0,0,9, 0,0,0,20,
            OP_LIT, 0,0,0,18, 0,0,0,1, 'a',
            OP_ABORT,
            OP_LIT, 0,0,0,29, 0,0,0,1, 'b',
            OP_ACCEPT
        };
        size_t pos = 0;
        bool ok = run_bc(bc, sizeof(bc), "abc", 3, &pos);
...
void test_pattern_succeed_suite(void) {
    ...
    /* 'a' SUCCEED LEN(10) on "abc": succeeds immediately at pos=1 */
    {
        ...
        bc[0]  = OP_LIT;        patch_u32(bc, 1, 9);   patch_u32(bc, 5, 1);  bc[9]  = 'a';
        bc[10] = OP_SUCCEED;
        bc[11] = OP_LIT;        patch_u32(bc, 12, 20); patch_u32(bc, 16, 10);
    ...
    /* SUCCEED at start of string succeeds immediately */
    {
        ...
        memset(bc, 0, sizeof(bc));
        bc[0] = OP_SUCCEED;
        bc[1] = OP_LIT;         patch_u32(bc, 2, 10); patch_u32(bc, 6, 10);
