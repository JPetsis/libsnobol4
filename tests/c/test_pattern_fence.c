...
static bool run_bc(const uint8_t *bc, size_t bc_len,
...
void test_pattern_fence_suite(void) {
    ...
    /* OP_FENCE test: after fence, no backtracking choice points remain */
    {
        ...
        uint8_t bc[] = {
            OP_SPLIT,
              0, 0, 0,  9,  /* a = offset 9 (FENCE) */
              0, 0, 0, 17,  /* b = offset 17 (FAIL) */
            /* 9: */ OP_FENCE,
            /* 10: */ OP_ACCEPT,
            /* 11 (alt=17 but this layout is demonstration): */
            OP_FAIL,
        };
        /* We won't actually use the alt branch – just demonstrate FENCE clears choices */
        size_t pos = 0;
    ...
    /* OP_RPOS test: succeed only at pos == len - n */
    {
        ...
        uint8_t bc[] = {
            OP_LEN,
              (uint8_t)(len_val >> 24), (uint8_t)(len_val >> 16),
              (uint8_t)(len_val >>  8), (uint8_t)(len_val),
            OP_RPOS,
              (uint8_t)(rpos_n >> 24), (uint8_t)(rpos_n >> 16),
              (uint8_t)(rpos_n >>  8), (uint8_t)(rpos_n),
            OP_ACCEPT
        };
        size_t pos = 0;
        ...
        uint8_t bc2[] = {
            OP_LEN,
              (uint8_t)(len_val2 >> 24), (uint8_t)(len_val2 >> 16),
              (uint8_t)(len_val2 >>  8), (uint8_t)(len_val2),
            OP_RPOS,
              (uint8_t)(rpos_n >> 24), (uint8_t)(rpos_n >> 16),
              (uint8_t)(rpos_n >>  8), (uint8_t)(rpos_n),
            OP_ACCEPT
    ...
    /* OP_RTAB test: advance to len-n position */
    {
        ...
        uint8_t bc[] = {
            OP_CAP_START, 0,
            OP_RTAB,
              (uint8_t)(rtab_n >> 24), (uint8_t)(rtab_n >> 16),
              (uint8_t)(rtab_n >>  8), (uint8_t)(rtab_n),
            OP_CAP_END, 0,
            OP_ACCEPT
        };
        size_t pos = 0;
        ...
        uint8_t bc2[] = {
            OP_RTAB,
              (uint8_t)(rtab0 >> 24), (uint8_t)(rtab0 >> 16),
              (uint8_t)(rtab0 >>  8), (uint8_t)(rtab0),
            OP_ACCEPT
        };
