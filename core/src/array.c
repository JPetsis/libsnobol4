...
static bool array_resize(snobol_array_t *array, size_t new_capacity) {
    ...
    array_entry_t *new_entries = (array_entry_t *)snobol_calloc(new_capacity, sizeof(array_entry_t));
    if (!new_entries) {
        return false;
    }
...
snobol_array_t *snobol_array_create(int32_t bounds_hint) {
    snobol_array_t *array = (snobol_array_t *)snobol_calloc(1, sizeof(snobol_array_t));
    if (!array) {
        return nullptr;
    }
    ...
    SNOBOL_LOG("snobol_array_create: array=%p bounds_hint=%d", (void*)array, bounds_hint);
    return array;
}
...
snobol_array_t *snobol_array_retain(snobol_array_t *array) {
    ...
    array->refcount++;
    SNOBOL_LOG("snobol_array_retain: array=%p refcount=%u", (void*)array, array->refcount);
    return array;
}
...
void snobol_array_release(snobol_array_t *array) {
    ...
    array->refcount--;
    SNOBOL_LOG("snobol_array_release: array=%p refcount=%u", (void*)array, array->refcount);
...
bool snobol_array_set(snobol_array_t *array, int32_t key, const char *value) {
    ...
    SNOBOL_LOG("snobol_array_set: array=%p key=%d value='%s'",
               (void*)array, key, value ? value : "(null)");
    ...
    while (array->entries[idx].active) {
        if (array->entries[idx].hash == hash &&
            array->entries[idx].key == key) {
            if (value) {
...
char **snobol_array_values(const snobol_array_t *array, size_t *out_count) {
    ...
    for (size_t i = 0; i < array->capacity; i++) {
        if (array->entries[i].active && array->entries[i].value) {
            values[count] = (char *)snobol_malloc(strlen(array->entries[i].value) + 1);
            if (values[count]) {
