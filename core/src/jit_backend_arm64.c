...
static inline uint32_t A64_TBZ(uint32_t rt, uint32_t bit, uint32_t imm) {
  return 0xb6000000u | (rt & 31u) | ((bit & 0x1fu) << 19) |
         (((bit >> 5) & 1u) << 31) | ((imm & 0x3fffu) << 5);
}
static inline uint32_t A64_TBNZ(uint32_t rt, uint32_t bit, uint32_t imm) {
  return 0xb7000000u | (rt & 31u) | ((bit & 0x1fu) << 19) |
         (((bit >> 5) & 1u) << 31) | ((imm & 0x3fffu) << 5);
}
...
static void emit_ubfx_w(jit_region_t *js, int rd, int rn, int lsb, int width) {
  emit_instr(js, 0x53000000 | (rd & 31) | ((rn & 31) << 5) | (lsb << 10) |
                     ((lsb + width - 1) << 16));
}
static void emit_patch_b_cond(uint32_t *p, uint32_t *target,
...
static void emit_patch_tbz(uint32_t *p, uint32_t *target,
                           uint32_t base_opcode) {
  intptr_t diff = target - p;
...
#define EMIT_CALLOUT_SAVE_RET(js) emit_instr((js), A64_MOV_X_X(9, 0))
 ...
 * A block ends when we hit SPLIT, JMP, GOTO, ACCEPT, FAIL, REPEAT_*, or
 * end-of-IR.
 */
...
#define EMIT_VOID_CALLOUT(js_, fn_ptr_)                                        \
  do {                                                                         \
    ...
    case JIT_IR_BREAKX: {
      ...
      *tbz_instr = A64_TBZ(4, 6, (js->p - tbz_instr));
      ...
      if (ins->opcode == JIT_IR_SPAN) {
        ...
        emit_instr(js, A64_CMP_X_X(2, 3));
        uint32_t *loop_ge_done = js->p;
        emit_instr(js, A64_B_GE(0));
        emit_instr(js, A64_LDRB_W_X_X(4, 1, 2));
        emit_instr(js, A64_MOV_W_IMM(7, 127));
        emit_instr(js, A64_CMP_W_W(4, 7));
        uint32_t *loop_hi_done = js->p;
        emit_instr(js, A64_B_HI(0));
        uint32_t *loop_tbz = js->p;
        emit_instr(js, 0);
        emit_ubfx_w(js, 7, 4, 0, 6);
        emit_instr(js, A64_LSR_X_X_X(8, 6, 7));
        uint32_t *loop_bit64_done = js->p;
        emit_instr(js, A64_TBZ(8, 0, 0));
        emit_instr(js, A64_MOV_X_IMM(7, 1));
        emit_instr(js, A64_ADD_X_X_X(2, 2, 7));
        ...
        *loop_tbz = A64_TBZ(4, 6, (js->p - loop_tbz));
        ...
        emit_patch_tbz(loop_bit64_done, span_done,
                       *loop_bit64_done & 0xfff8001f);
  ...
  if (use_cfg) {
    ...
    for (size_t f = 0; f < fail_patch_count; f++) {
      ...
      if ((orig & 0xff000000) == 0x54000000)
        emit_patch_b_cond(fail_patches[f].instr_p, fail_stub,
                          orig & 0xff00001f);
