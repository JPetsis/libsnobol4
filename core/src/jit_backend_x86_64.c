...
/* Encode REX prefix byte: W=64-bit operand, R=extended reg.modrm, X=extended SIB index, B=extended rm/base */
static uint8_t rex(int w, int r, int x, int b) {
    return (uint8_t)(0x40u | (w ? 8u : 0u) | (r ? 4u : 0u) | (x ? 2u : 0u) | (b ? 1u : 0u));
}

/* Build ModRM byte: mod=addressing mode, reg=opcode/register, rm=register/memory */
...
/* Emit REX.W + ModRM.rm with no REX.R (for instructions using the reg field differently) */
...
static void x64_cmp_rm32_imm32(jit_region_t *out, int rm, int32_t imm) {
    int need_rex = rm > 7;
    if (need_rex) emit_byte(out, rex(0, 0, 0, 1));
    emit_byte(out, 0x81);
...
static void x64_load_mr_rsp(jit_region_t *out, int reg, int base, int32_t disp) {
...
static void x64_loadb_mr(jit_region_t *out, int reg, int base, int32_t disp) {
    ...
    if (need_rex_r || need_rex_b) emit_byte(out, rex_byte);
    emit_byte(out, 0x0F);
    emit_byte(out, 0xB6);
    if (disp == 0 && base != X64_RBP && base != X64_RSP) {
        emit_byte(out, modrm(MOD_DISP0, reg & 7, base & 7));
...
static void x64_load_sib(jit_region_t *out, int reg, int base, int index, int scale, int32_t disp) {
    emit_rex_w(out, reg, base);
    emit_byte(out, 0x8B);
    if (disp == 0 && base != X64_RBP) {
...
/* Jcc rel8: 0x70+cc + 1 byte offset (cc=0 for JO, 1 for JNO, ..., 4 for JE, 5 for JNE, ...)
 * JE  = 0x74 (cc=4)
 * JNE = 0x75 (cc=5)
 * JL  = 0x7C (cc=12)
 * JGE = 0x7D (cc=13)
...
/* RET: 0xC3 */
static void x64_ret(jit_region_t *out) {
    emit_byte(out, 0xC3);
}
...
static void x64_push_r(jit_region_t *out, int reg) {
    if (reg > 7) emit_byte(out, rex(0, 0, 0, 1));
    x64_push(out, reg);
}
...
#ifdef SNOBOL_DYNAMIC_PATTERN
static bool snobol_jit_helper_table_get(VM *vm, uint16_t table_id, uint8_t key_reg, uint8_t dest_reg) {
    (void)dest_reg;
    snobol_table_t *table = vm_get_table(vm, table_id);
...
static bool snobol_jit_helper_table_set(VM *vm, uint16_t table_id, uint8_t key_reg, uint8_t val_reg) {
    snobol_table_t *table = vm_get_table(vm, table_id);
    if (!table) return false;
    if (key_reg   >= MAX_CAPS || vm->cap_end[key_reg]   <= vm->cap_start[key_reg]) return false;
    if (val_reg >= MAX_CAPS || vm->cap_end[val_reg] <= vm->cap_start[val_reg]) return false;
    size_t kl = vm->cap_end[key_reg]   - vm->cap_start[key_reg];
    ...
    memcpy(key, vm->s + vm->cap_start[key_reg],   kl); key[kl] = '\0';
    memcpy(val, vm->s + vm->cap_start[val_reg], vl); val[vl] = '\0';
...
static void ir_cfg_scan_block(const jit_ir_region_t *ir, size_t ir_start_idx,
    ...
    if (ir_start_idx >= ir->count) {
        blk->start_ip = 0; blk->term_ip = 0; blk->next_ip = 0;
...
static int ir_cfg_build(const jit_ir_region_t *ir, JitCfg *cfg) {
    cfg->count = 0; cfg->has_backward = false;
...
static uint8_t *x64_emit_block_ops(jit_region_t *out, const jit_ir_region_t *ir,
                                    ...
                                    size_t fail_patch_cap,
                                    size_t start_ip, size_t term_ip,
                                    VM *vm) {
    (void)fail_patches; (void)fp_count; (void)fail_patch_cap;
    (void)start_ip; (void)term_ip;
    ...
    for (size_t i = ir_start; i < ir_end; i++) {
        ...
        case JIT_IR_JMP: {
            ...
            if (backward) {
                int32_t diff = (int32_t)(intptr_t)((uint8_t *)out->p - (uint8_t *)out->code_start);
                (void)diff;
                /* Backward JMP to loop start — just continue linear. Real loop happens via
                 * CFG exit + re-enter. For now patch later. */
                ...
                if (sp) {
                    sp->instr_p = patch;
                    sp->target_ip = target;
                    /* We store in out->code_start for now — but we can't allocate dynamically.
                     * For simplicity, use linear pass and no loop unrolling in stub. */
                    snobol_free(sp);
                }
            } else {
        ...
        case JIT_IR_CAP_START: {
            ...
            x64_store_mr(out, X64_RBX, X64_RDI, offsetof(VM, cap_start) + (int32_t)cap_reg * 8);
            break;
        }

        case JIT_IR_CAP_END: {
            uint8_t cap_reg2 = ins->u.cap.reg;
            x64_store_mr(out, X64_RBX, X64_RDI, offsetof(VM, cap_end) + (int32_t)cap_reg2 * 8);
            break;
        }
        ...
        case JIT_IR_ASSIGN: {
            ...
            x64_load_mr(out, X64_RAX, X64_RBX, offsetof(VM, cap_start) + (int32_t)cap_reg3 * 8);
            x64_store_mr(out, X64_RBX, X64_RAX, offsetof(VM, var_start) + (int32_t)var * 8);
            x64_load_mr(out, X64_RAX, X64_RBX, offsetof(VM, cap_end) + (int32_t)cap_reg3 * 8);
            x64_store_mr(out, X64_RBX, X64_RAX, offsetof(VM, var_end) + (int32_t)var * 8);
...
static void *x86_64_lower(const jit_ir_region_t *ir, VM *vm, jit_region_t *out) {
    ...
    if (use_cfg) {
        ...
        out->p = (uint32_t *)x64_emit_block_ops(out, ir, 0, ir->count,
                                                  fail_patches, &fp_count, 64,
                                                  0, 0, vm);
    }
