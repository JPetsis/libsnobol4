...
static inline uint32_t rv64_r(uint32_t op, uint32_t rd, uint32_t funct3,
  ...
  return op | (rd << 7) | (funct3 << 12) | (rs1 << 15) | (rs2 << 20) |
         (funct7 << 25);
}
...
static inline uint32_t rv64_i(uint32_t op, uint32_t rd, uint32_t funct3,
  ...
  return op | (rd << 7) | (funct3 << 12) | (rs1 << 15) |
         ((imm11_0 & 0xFFF) << 20);
}

/* S-type: op | imm11_5 | rs2 | funct3 | rs1 | imm4_0 */
static inline uint32_t rv64_s(uint32_t op, uint32_t funct3, uint32_t rs1,
                              uint32_t rs2, uint32_t imm) {
  uint32_t imm11_5 = (imm >> 5) & 0x7F;
  uint32_t imm4_0 = imm & 0x1F;
  return op | (imm11_5 << 25) | (rs2 << 20) | (funct3 << 12) | (rs1 << 15) |
         (imm4_0 << 7);
}

/* B-type: op | funct3 | rs1 | rs2 | imm[12|10:5|4:1|11] */
static inline uint32_t rv64_b(uint32_t op, uint32_t funct3, uint32_t rs1,
                              uint32_t rs2, uint32_t imm) {
  uint32_t b12 = (imm >> 12) & 1;
  uint32_t b11 = (imm >> 11) & 1;
  uint32_t b105 = (imm >> 5) & 0x3F;
  uint32_t b41 = (imm >> 1) & 0x0F;
  return op | (b12 << 31) | (b105 << 25) | (rs2 << 20) | (funct3 << 12) |
         (rs1 << 15) | (b41 << 8) | (b11 << 7);
}
...
static inline uint32_t rv64_j(uint32_t op, uint32_t rd, uint32_t imm) {
  ...
  return op | (rd << 7) | (j20 << 31) | (j101 << 21) | (j11 << 20) |
         (j1912 << 12);
}
...
static inline uint32_t rv64_srai(uint32_t rd, uint32_t rs1, uint32_t shamt) {
  return rv64_i(RV_OP_OP_IMM, rd, RV_F3_SRL, rs1,
                (shamt & 0x3F) | (0x400 << 20));
}
 ...
 * Upper half: emit_mov_imm32 + SLLI 32.  Lower half: emit_mov_imm32 + SLLI 32 +
 * SRLI 32 (zero-extend).  Then ADD to combine.
 *
...
static void emit_mov_w16(jit_region_t *js, int rd, uint32_t val) {
  if (val <= 2047) {
    ...
    if (val & 0xFFF)
      emit_instr(js, rv64_addi(rd, rd, val & 0xFFF));
  }
}
...
#define EMIT_CALLOUT_RESTORE(js)                                               \
  do {                                                                         \
...
#define EMIT_VOID_CALLOUT(js_, fn_ptr_, pc_)                                   \
  do {                                                                         \
...
#define EMIT_BOOL_CALLOUT(js_, fn_ptr_, fp_, fpc_, cur_)                       \
  do {                                                                         \
...
static bool emit_block_ops_ir(jit_region_t *js, const jit_ir_region_t *ir,
  ...
  for (size_t i = 0; i < ir->count; i++) {
    ...
    case JIT_IR_BREAKX: {
      ...
      uint32_t *map_done = js->p;
      emit_instr(js, rv64_jal(RV64_ZERO, 0));
      *use_map1 = rv64_bne(RV64_T6, RV64_ZERO,
                           (uint32_t)((intptr_t)js->p - (intptr_t)use_map1));
      emit_instr(js, rv64_andi(RV64_T6, RV64_T3, 63));
      emit_instr(js, rv64_srl(RV64_T6, RV64_T5, RV64_T6));
      emit_instr(js, rv64_andi(RV64_T6, RV64_T6, 1));
      *map_done =
          rv64_jal(RV64_ZERO, (uint32_t)((intptr_t)js->p - (intptr_t)map_done));
    ...
    case JIT_IR_SPAN: {
      ...
      emit_instr(js, rv64_andi(RV64_T6, RV64_T3, 0x40));
      uint32_t *sp_use_map1 = js->p;
      emit_instr(js, rv64_bne(RV64_T6, RV64_ZERO, 0));
      emit_instr(js, rv64_andi(RV64_T6, RV64_T3, 63));
      emit_instr(js, rv64_srl(RV64_T6, RV64_T4, RV64_T6));
      emit_instr(js, rv64_andi(RV64_T6, RV64_T6, 1));
      uint32_t *sp_map_done = js->p;
      emit_instr(js, rv64_jal(RV64_ZERO, 0));
      *sp_use_map1 =
          rv64_bne(RV64_T6, RV64_ZERO,
                   (uint32_t)((intptr_t)js->p - (intptr_t)sp_use_map1));
      emit_instr(js, rv64_andi(RV64_T6, RV64_T3, 63));
      emit_instr(js, rv64_srl(RV64_T6, RV64_T5, RV64_T6));
      emit_instr(js, rv64_andi(RV64_T6, RV64_T6, 1));
      *sp_map_done = rv64_jal(
          RV64_ZERO, (uint32_t)((intptr_t)js->p - (intptr_t)sp_map_done));
      ...
      emit_instr(js, rv64_bne(RV64_T6, RV64_ZERO, 0));
      emit_instr(js, rv64_andi(RV64_T6, RV64_T3, 0x40));
      uint32_t *sp_use1 = js->p;
      emit_instr(js, rv64_bne(RV64_T6, RV64_ZERO, 0));
      emit_instr(js, rv64_andi(RV64_T6, RV64_T3, 63));
      emit_instr(js, rv64_srl(RV64_T6, RV64_T4, RV64_T6));
      emit_instr(js, rv64_andi(RV64_T6, RV64_T6, 1));
      uint32_t *sp_done2 = js->p;
      emit_instr(js, rv64_jal(RV64_ZERO, 0));
      *sp_use1 = rv64_bne(RV64_T6, RV64_ZERO,
                          (uint32_t)((intptr_t)js->p - (intptr_t)sp_use1));
      emit_instr(js, rv64_andi(RV64_T6, RV64_T3, 63));
      emit_instr(js, rv64_srl(RV64_T6, RV64_T5, RV64_T6));
      emit_instr(js, rv64_andi(RV64_T6, RV64_T6, 1));
      *sp_done2 =
          rv64_jal(RV64_ZERO, (uint32_t)((intptr_t)js->p - (intptr_t)sp_done2));
      sp_lpatches[sp_np++] = js->p;
      emit_instr(js, rv64_beq(RV64_T6, RV64_ZERO, 0));
      emit_instr(js, rv64_addi(RV64_POS, RV64_POS, 1));
      ...
      for (int pi = 0; pi < sp_np; pi++) {
        ...
        *sp_lpatches[pi] =
            rv64_b(opb, f3, rs1b, rs2b, (uint32_t)(foff & 0x1FFF));
      }
      break;
    }
    ...
    case JIT_IR_ASSIGN: {
      ...
      emit_instr(js,
                 rv64_sd(RV64_VM, RV64_T4, offsetof(VM, var_start) + var * 8));
      emit_ld_from_vm(js, RV64_T4, offsetof(VM, cap_end) + reg * 8);
      emit_instr(js,
                 rv64_sd(RV64_VM, RV64_T4, offsetof(VM, var_end) + var * 8));
 ...
 * ========================================================================= */
static void *riscv64_lower(const jit_ir_region_t *ir, VM *vm,
                           jit_region_t *out) {
  if (!ir || !vm || !out)
    return nullptr;
  ...
  if (use_cfg) {
    ...
    for (int bi = 0; bi < cfg.count; bi++) {
      ...
      case BLOCK_TERM_JMP_BWD: {
        ...
        *bail_patch =
            rv64_beq(RV64_LOOP, RV64_ZERO, (uint32_t)(bail_diff & 0x1FFF));
        break;
      }
      case BLOCK_TERM_EXIT:
        emit_cfg_bailout_riscv64(out, blk->term_ip);
    ...
    for (size_t f = 0; f < fail_patch_count; f++) {
      ...
      intptr_t stub_diff =
          (intptr_t)fail_stub - (intptr_t)fail_patches[f].instr_p;
      uint32_t opcode = orig & 0x7F;
      if (opcode == RV_OP_BRANCH) {
        *fail_patches[f].instr_p =
            rv64_b(opcode, (orig >> 12) & 7, (orig >> 15) & 0x1F,
                   (orig >> 20) & 0x1F, (uint32_t)(stub_diff & 0x1FFF));
      }
    }
