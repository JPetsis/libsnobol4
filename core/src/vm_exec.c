...
op_eval:
...
#endif
    {
      ...
      if (fn > SNOBOL_FN_NONE && fn < SNOBOL_FN_MAX) {
        ...
        case SNOBOL_FN_INTEGER:
          ok = snobol_integer(cap_s, cap_l);
          break;
        case SNOBOL_FN_REAL:
          ok = snobol_real(cap_s, cap_l);
          break;
        case SNOBOL_FN_NUMERIC:
          ok = snobol_numeric(cap_s, cap_l);
          break;
      ...
      if (r < MAX_CAPS && vm->cap_end[r] >= vm->cap_start[r] &&
        ...
        if (mapped_type == SNBL_FMT_UPPER) {
          ...
          for (size_t i = 0; i < len; ++i)
            tmp[i] =
                (data[i] >= 'a' && data[i] <= 'z') ? data[i] - 32 : data[i];
          if (vm->out)
...
#endif
    {
      ...
      if (r < MAX_CAPS && vm->cap_end[r] >= vm->cap_start[r] &&
        ...
        if (format_type == SNBL_FMT_UPPER) {
          ...
          if (vm->emit_fn)
            vm->emit_fn(tmp, (size_t)n, vm->emit_udata);
        } else if (format_type == SNBL_FMT_LPAD ||
                   format_type == SNBL_FMT_RPAD) {
...
op_dynamic:
...
#endif
    {
      ...
      dynamic_pattern_t *pattern =
          dynamic_pattern_cache_get(vm->dyn_cache, vm->dyn_pending_source,
