 ...
 *   1. Contains no EVAL, ASSIGN, EMIT_*, DYNAMIC, TABLE_GET/SET, GOTO*, LABEL ops
 *      (side-effect-free during matching).
    ...
    while (ip < bc_len) {
        ...
        switch (op) {
            ...
            case OP_NOP:
                ...
                break;
            case OP_REPEAT_INIT:
                if (ip + 13 > bc_len) return false;
                ip += 13;
                break;
            case OP_REPEAT_STEP:
                if (ip + 5 > bc_len) return false;
                ip += 5;
                break;
            default:
...
void snobol_search_derive_meta(const uint8_t *bc, size_t bc_len,
    ...
    if (op0 == OP_LIT && ip + 9 <= bc_len) {
        ...
        if (lit_off < bc_len && lit_off + lit_len <= bc_len && lit_len > 0) {
            size_t prefix_len = lit_len < SNOBOL_SEARCH_MAX_PREFIX
                                ? lit_len : SNOBOL_SEARCH_MAX_PREFIX;
    ...
    /* OP_BREAK / OP_BREAKX: scan past delimiter set */
        ...
        if (ranges) {
            out->ascii_class_only = ranges_to_ascii_bitmap(ranges, count,
                                                            out->class_bitmap);
        }
    }
     ...
     */
        ...
        #define CHECK_SINGLE_LIT(off) do {                              \
            if ((off) + 10 > bc_len) { all_single = false; break; }    \
            if (bc[(off)] != OP_LIT) { all_single = false; break; }     \
            uint32_t _off2 = search_read_u32(bc, (off) + 1);           \
            uint32_t _len  = search_read_u32(bc, (off) + 5);           \
            if (_len != 1) { all_single = false; break; }              \
            if (_off2 >= bc_len) { all_single = false; break; }        \
            uint8_t _byte = bc[_off2];                                  \
            if (bc[(off) + 9] != OP_ACCEPT) { all_single = false; break; } \
        ...
        if (all_single) CHECK_SINGLE_LIT(branch_b);
        ...
        if (all_single && alt_count > 0) {
            ...
            for (size_t i = 0; i < alt_count; i++) {
                ...
                if (b < 64)  out->candidate_bitmap[0] |= (1ULL << b);
                else if (b < 128) out->candidate_bitmap[1] |= (1ULL << (b - 64));
            }
            out->has_first_byte   = (alt_count == 1);
            if (out->has_first_byte) out->first_byte = alt_bytes[0];
            out->always_consumes  = true;
            out->may_match_empty  = false;
        }
    }
    ...
    if (out->automaton_eligible &&
        (out->is_break_family || out->is_span_family) &&
 ...
 * --------------------------------------------------------------------------- */
static inline void search_reset_vm(VM *vm,
 ...
 * byte.
 * --------------------------------------------------------------------------- */
static bool search_break_accelerated(VM *vm,
                                      const char *subject, size_t subject_len,
                                      size_t start_offset,
    ...
    while (offset <= subject_len) {
        ...
        while (scan < subject_len) {
            ...
            if (c > 127 || bitmap_test(bmap, c)) break;
            scan++;
        }
        ...
        if (diag) diag->candidates_tested++;
 ...
 * the bitmap.
 * --------------------------------------------------------------------------- */
static bool search_span_accelerated(VM *vm,
                                     const char *subject, size_t subject_len,
                                     size_t start_offset,
                                     const snobol_search_meta_t *meta,
                                     snobol_search_result_t *out_result,
    ...
    while (offset < subject_len) {
        ...
        if (c > 127 || !bitmap_test(bmap, c)) {
            if (diag) diag->candidates_skipped++;
            offset++;
            continue;
        }
 ...
 * --------------------------------------------------------------------------- */
static bool search_literal_accelerated(VM *vm,
                                        const char *subject, size_t subject_len,
                                        size_t start_offset,
                                        const snobol_search_meta_t *meta,
                                        snobol_search_result_t *out_result,
    ...
    while (offset <= subject_len) {
        ...
        if (meta->literal_prefix_len == 1) {
            ...
            found = (const char *)memmem(hay, haylen,
                                          meta->literal_prefix,
        ...
        if (!found) {
            ...
            if (diag) diag->candidates_skipped += haylen;
            break;
        }
 ...
 * --------------------------------------------------------------------------- */
static bool search_bitmap_accelerated(VM *vm,
                                       const char *subject, size_t subject_len,
                                       size_t start_offset,
                                       const snobol_search_meta_t *meta,
                                       snobol_search_result_t *out_result,
 ...
 * for the common case.  Falls back to the general VM on any branching op (SPLIT).
 *
 ...
 * --------------------------------------------------------------------------- */
static bool search_automaton_try(VM *vm,
                                  const char *subject, size_t subject_len,
                                  size_t offset,
                                  snobol_search_result_t *out_result) {
 ...
 *   5. Automaton-eligible               → automaton path (general VM + eligible flag)
...
bool snobol_search_exec(VM *vm,
    ...
    if (start_offset > subject_len) return false;
    ...
    if (meta->is_break_family && meta->ascii_class_only) {
        ...
        return search_break_accelerated(vm, subject, subject_len,
                                         start_offset, meta, out_result, diag);
    }
    ...
    if (meta->is_span_family && meta->ascii_class_only) {
        ...
        return search_span_accelerated(vm, subject, subject_len,
                                        start_offset, meta, out_result, diag);
    }
    ...
    if (meta->has_literal_prefix && meta->literal_prefix_len > 0) {
        ...
        return search_literal_accelerated(vm, subject, subject_len,
                                           start_offset, meta, out_result, diag);
    }
    ...
    if (meta->has_candidate_bitmap && meta->is_single_char_alt) {
        ...
        return search_bitmap_accelerated(vm, subject, subject_len,
                                          start_offset, meta, out_result, diag);
    }
    ...
    if (meta->automaton_eligible) {
        ...
        while (offset <= subject_len) {
            if (diag) diag->candidates_tested++;
            if (diag) diag->automaton_tests++;
            bool ok = search_automaton_try(vm, subject, subject_len, offset, out_result);
            if (ok) return true;
