...
typedef struct {
    bool   success;
    size_t match_start;  /**< Byte offset of match start within the original subject */
    size_t match_end;    /**< Byte offset just past the match end in the original subject */
...
typedef enum {
    ...
    SNOBOL_SEARCH_SKIP_LITERAL        = 1,  /**< Skipped by literal prefix scan (memmem) */
    ...
    SNOBOL_SEARCH_SKIP_JIT_COLD       = 6,  /**< JIT skip: below search-mode hotness threshold */
    ...
    SNOBOL_SEARCH_BAILOUT_UNSUPPORTED = 9,  /**< JIT bailout: unsupported control flow */
    SNOBOL_SEARCH_BAILOUT_SAFETY      = 10, /**< JIT bailout: safety fallback triggered */
    SNOBOL_SEARCH_BAILOUT_CANDIDATE   = 11, /**< JIT bailout: expected candidate rejection */
...
/**
 * Execute a single search over [subject, subject+subject_len) from start_offset,
 * returning the first non-overlapping match using one native C control loop.
 *
 ...
 * @param meta          Optional pre-derived metadata for acceleration (NULL = derive inline)
 * @param out_result    Output: match position (always written; success=false on no match)
 ...
 */
SNOBOL_NODISCARD bool snobol_search_exec(VM *vm,
