 ...
 *
 * The IR sits between the VM bytecode lifter and the target-specific code emitter.
 * Pipeline:
 *   VM bytecodes → [Lifter] → jit_ir_region_t → [Optimiser passes] → [Backend lowerer] → machine code
 *
 * Design decisions:
 *  - Linear, flat instruction array (not SSA) — sufficient for the RISC-like targets.
 *  - Virtual registers encoded as uint8_t indices (max 256 per region).
 *  - All operands are pre-decoded by the lifter; the backend must NOT re-parse raw bytecode.
 *  - Memory: IR arrays are heap-allocated and freed immediately after the backend lowers them.
 */
#pragma once
...
/* Forward-declare VM to avoid circular dependency.  The lifter needs the full VM
 * definition but we keep the header dependency explicit. */
#ifdef SNOBOL_JIT
#include "snobol/vm.h"
#endif
 ...
 * ------------------------------------------------------------------------- */
#define JIT_IR_FLAG_SIDE_EFFECT  0x01u  /**< Instruction has observable side effects */
#define JIT_IR_FLAG_CALLOUT      0x02u  /**< Instruction performs a C helper call-out */
#define JIT_IR_FLAG_PURE         0x04u  /**< Instruction is pure; removable if dst unused */
...
typedef struct {
    ...
    uint8_t         dst_reg;    /**< Output virtual register (JIT_IR_VREG_NONE = no output) */
    ...
    union {
        ...
        struct { uint16_t set_id; } set;

        /* JIT_IR_CAP_START / CAP_END */
        struct { uint8_t reg; } cap;

        /* JIT_IR_ASSIGN */
        struct { uint16_t var; uint8_t reg; } assign;

        /* JIT_IR_LEN */
        struct { uint32_t n; } len;

        /* JIT_IR_ANCHOR */
        struct { uint8_t type; } anchor;

        /* JIT_IR_SPLIT: fork with two successors */
        struct { size_t target_a; size_t target_b; } split;

        /* JIT_IR_JMP / JIT_IR_GOTO: unconditional branch */
        struct { size_t target; } jmp;

        /* JIT_IR_GOTO_F: conditional goto (label_id retained for debugging) */
        struct { uint16_t label_id; size_t target; } goto_f;

        /* JIT_IR_RPOS, JIT_IR_RTAB */
        struct { uint32_t n; } rpos_rtab;

        /* JIT_IR_LABEL */
        struct { uint16_t label_id; } label;

        /* JIT_IR_EMIT_LITERAL */
        struct { const uint8_t *data; uint32_t len; uint32_t offset; } emit_lit;

        /* JIT_IR_EMIT_CAPTURE */
        struct { uint8_t reg; } emit_cap;

        /* JIT_IR_EMIT_EXPR */
        struct { uint8_t reg; uint8_t expr_type; } emit_expr;

        /* JIT_IR_EMIT_FORMAT */
        struct { uint8_t reg; uint8_t fmt_type; uint16_t width; uint8_t fill_char; } emit_fmt;
        ...
        /* JIT_IR_TABLE_GET */
        struct { uint16_t table_id; uint8_t key_reg; uint8_t dest_reg; } tget;

        /* JIT_IR_TABLE_SET */
        struct { uint16_t table_id; uint8_t key_reg; uint8_t val_reg; } tset;

        /* JIT_IR_BAL */
        struct { uint32_t open_cp; uint32_t close_cp; } bal;

        /* JIT_IR_EVAL */
        struct { uint16_t fn_id; uint8_t reg; } eval;
...
typedef struct {
    ...
    uint16_t        vreg_next;     /**< Next virtual register to allocate (1-based; 0 = NONE) */
 ...
 * @return          Heap-allocated IR region; caller must free with jit_ir_region_free().
